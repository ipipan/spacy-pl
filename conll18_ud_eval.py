...
#   - WHOLE_XTAG: a custom metric, calculates whether the entire morphosyntactic tag matches
...
def load_conllu(file):
    ...
    class UDWord:
        def __init__(self, span, columns, is_multiword):
            ...
            #LETS DO THE SAME FOR XPOS @rtuora
            self.columns[XPOS] = columns[XPOS].split(":")[0]
            self.columns[10] = columns[XPOS] # @rtuora
    ...
    while True:
        ...
        if not columns[FORM]:
            print('raise UDError("There is an empty FORM in the CoNLL-U file")')
            pass#raise UDError("There is an empty FORM in the CoNLL-U file")
...
def evaluate(gold_ud, system_ud):
    ...
    return {
        ...
        "WHOLE_XTAG": alignment_score(alignment, lambda w, _: w.columns[10]),
